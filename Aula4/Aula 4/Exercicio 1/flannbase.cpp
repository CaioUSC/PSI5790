...
int main() {
  Mat_<Vec3b> ax=imread("janei.pgm",1);
  Mat_<uchar> ay=imread("janei-1.pgm",0);
  Mat_<Vec3b> qx=imread("julho.pgm",1);
  ...
  for (int l=0; l<qp.rows; l++)
    for (int c=0; c<qp.cols; c++) {
      ...
      ind.knnSearch(query,indices,dists,1,flann::SearchParams(16));
      qp(l,c)=saidas(indices[0]);
    }
  imwrite("f1-flann2.png",qp);
}
